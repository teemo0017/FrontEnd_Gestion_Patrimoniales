...
export class DashboardComponent implements OnInit {
  ...
  descargarPdf() {
    const body = this.bienInfo!.map((item) => [item.nombre, item.descripcion, item.numeroSerie, item.marca, item.modelo,item.categoria,item.usuarioResponsable,item.usuarioCreador]);

    const head: string[][] = [['Nombre', 'Descripcion', 'Serie', 'Marca', 'Modelo','Categoria','P Asignada','Registrador']];
